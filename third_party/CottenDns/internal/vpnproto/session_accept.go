...
//
// A server states its ceilings once, at handshake, and a cooperating client
// clamps itself to them. On a shared public server this is what stops one
// client configured with a huge ARQ window or a very aggressive ping interval
// from taking a disproportionate share.
//
// Two compatibility properties matter and both are load-bearing:
//
//   - The block is OPTIONAL. A server that configured no ceilings appends
//     nothing, and the payload is byte-for-byte what it has always been.
//   - The layout is byte-identical to MasterDnsVPN's, and sits after the base
//     payload at whichever session-ID width this build is running in. This
//     engine selects that width once at startup via ConfigureLegacySessionID
//     (see compat.go), so the offset is read from that global rather than
//     passed per call -- a client speaks exactly one wire format for its
//     lifetime.
// ==============================================================================
...
const (
	...
	// sessionPolicyScaledMin/Max bound the two sub-second fields transmitted as
	// a single byte. The range is fixed by the wire format and shared with
	// MasterDnsVPN; changing it would silently reinterpret values on the other
	// side.
...
// the wire: it would advertise ceilings of zero, which a client would clamp
// itself to death against.
func (p SessionAcceptClientPolicy) IsZero() bool {
	return p == SessionAcceptClientPolicy{}
}

// SessionAcceptBaseSize is the payload length before any policy block. It
// tracks the session-ID width this build is configured for.
func SessionAcceptBaseSize() int {
	return sessionIDLen + 6 // sid + cookie + compression + verify(4)
}

// EncodeSessionAccept builds the SESSION_ACCEPT payload at the configured
// width, appending the policy block only when one is configured.
func EncodeSessionAccept(sessionID uint16, cookie uint8, compressionPair uint8, verifyCode [4]byte, policy SessionAcceptClientPolicy) []byte {
	base := SessionAcceptBaseSize()
	size := base
	if !policy.IsZero() {
...
func DecodeSessionAcceptPolicy(payload []byte) (SessionAcceptClientPolicy, bool) {
	base := SessionAcceptBaseSize()
